...
export default function RegisterPage() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      toast.success(`Welcome to Splito, ${res.data.user.name}! 🎉`);
      navigate('/dashboard');
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-8 anim-slide-up">
          ...
          <h1 className="text-2xl font-bold text-white" style={{ fontFamily: 'DM Sans,sans-serif' }}>Create your account</h1>
        ...
        <div className="card anim-slide-up d200">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div><label className="label">Full Name</label><input name="name" value={form.name} onChange={handle} className="input" placeholder="Tanvir Ahmed" /></div>
            <div><label className="label">Email</label><input name="email" type="email" value={form.email} onChange={handle} className="input" placeholder="you@example.com" /></div>
            <div><label className="label">Password</label><input name="password" type="password" value={form.password} onChange={handle} className="input" placeholder="Min. 6 characters" /></div>
